...
def extract_last_vm_data(dead_host, pg_client):
    pg_cursor = pg_client.cursor()
    vm_dict = {}
    pg_cursor.execute("SELECT * FROM vm_status WHERE node_name = '%s';" % dead_host)
...
def extract_available_resources_data(dead_host, pg_client):
    pg_cursor = pg_client.cursor()
